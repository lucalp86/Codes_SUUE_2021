...
PositionY = linspace(air_gap*0.8, -air_gap*2, 15);
...
mi_setblockprop('Air', 0, 5*SizeMesh_Air, 0, 0, 0, 0);
mi_clearselected;
...
Field = zeros(14, Npoint_Bsampling,size(PositionY,2));
FluxLinkage = zeros(1 ,size(PositionY,2));   

for y = 1:size(PositionY,2)

    EccX = 0;
    EccY = PositionY(1,y);

    mi_selectgroup(1);
    mi_movetranslate(EccX, EccY)

    mi_analyze;
    
    mi_selectgroup(1);
    mi_movetranslate(-EccX, -EccY); % Restore the initial position
    ...
    for f = 1 : size(x_Bsampling,2)
        ...
        Field(1:14, f, y)=mo_getpointvalues(x_pos,y_pos);
        %	1: A_z           2: B_x      3: B_y
    ...
    FluxLinkage(1,y) = CircuitSolution(1,3);
        
end

figure;hold all;
plot(x_Bsampling, squeeze(Field(2, :, 1)));
plot(x_Bsampling, squeeze(Field(3, :, 1)));


figure;hold all;
plot(PositionY, FluxLinkage);

%%mi_modifycircprop('Coil_plus', 1, current_new);



% mi_selectgroup(1);
% 
% mi_movetranslate(0,-air_gap*5);

llllllllllllllllllllllllllllllll



We_F_TOT(:,rad)=R_field(1,rad)*span_skewing(1,ax)*trapz(teta_field,squeeze(EM_Sigma_TOT(:,rad,:)),2);
We_M_TOT(:,rad)=R_field(1,rad)*trapz(teta_field,squeeze(EM_Tau_TOT(:,rad,:)),2);
